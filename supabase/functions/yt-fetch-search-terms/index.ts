...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log('Fetching search terms for user:', userId);
    ...
    console.log(`Processing ${chunks.length} quarterly chunks for search terms`);
    ...
    for (const chunk of chunks) {
      try {
        console.log(`Fetching search terms for ${chunk.start} to ${chunk.end}`);
        ...
        if (result.rows && result.rows.length > 0) {
          ...
          if (insertError) {
            ...
            console.log(`Inserted ${records.length} search term records`);
          }
        }
