...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log('Fetching audience retention for user:', userId);
    ...
    console.log(`Processing ${chunks.length} quarterly chunks for retention`);
    ...
    for (const chunk of chunks) {
      try {
        console.log(`Fetching retention data for ${chunk.start} to ${chunk.end}`);
        ...
        if (result.rows && result.rows.length > 0) {
          ...
          if (insertError) {
            ...
            console.log(`Inserted ${records.length} retention records`);
          }
        }
