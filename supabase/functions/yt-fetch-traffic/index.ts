...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log('Fetching traffic sources for user:', userId);
    ...
    console.log(`Processing ${chunks.length} quarterly chunks for traffic sources`);
    ...
    for (const chunk of chunks) {
      try {
        // Fetch traffic source types
        console.log(`Fetching traffic source types for ${chunk.start} to ${chunk.end}`);
        
        const typesResult = await queryYouTubeAnalytics(
          token,
          channelId,
          chunk.start,
          chunk.end,
        ...
        if (typesResult.rows && typesResult.rows.length > 0) {
          ...
          if (insertError) {
            ...
            failedChunks.push({ chunk, type: 'types', error: insertError.message });
          } else {
            totalInserted += records.length;
            console.log(`Inserted ${records.length} traffic type records`);
          }
        }
        ...
        console.log(`Fetching traffic source details for ${chunk.start} to ${chunk.end}`);
        
        const detailResult = await queryYouTubeAnalytics(
          token,
          channelId,
          chunk.start,
          chunk.end,
        ...
        if (detailResult.rows && detailResult.rows.length > 0) {
          ...
          if (insertError) {
            console.error('Insert error:', insertError);
            failedChunks.push({ chunk, type: 'details', error: insertError.message });
          } else {
            totalInserted += records.length;
            console.log(`Inserted ${records.length} traffic detail records`);
          }
        }
