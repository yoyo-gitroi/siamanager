...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log('Fetching playlist analytics for user:', userId);
    ...
    console.log(`Processing ${chunks.length} quarterly chunks for playlists`);
    ...
    for (const chunk of chunks) {
      try {
        console.log(`Fetching playlist data for ${chunk.start} to ${chunk.end}`);
        ...
        const result = await queryYouTubeAnalytics(
          ...
          chunk.start,
          chunk.end,
          'views,estimatedMinutesWatched,playlistStarts,viewsPerPlaylistStart,averageTimeInPlaylist',
          'playlist',
          200
        );
        ...
        if (result.rows && result.rows.length > 0) {
          ...
          if (insertError) {
            ...
            console.log(`Inserted ${records.length} playlist records`);
          }
        }
